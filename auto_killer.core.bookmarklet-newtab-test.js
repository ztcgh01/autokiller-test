(function () {
  'use strict';
  const SCRIPT_VERSION = '2.23.13F-BMNT1';
  ...
  const BOOKMARKLET_NEWTAB = BOOKMARKLET_MODE && window.__AUTO_KILLER_BOOKMARKLET_NEWTAB__ === true;
  ...
  function openTransferTab() {
    if ((BOOKMARKLET_MODE && !BOOKMARKLET_NEWTAB) || ONECLICK_BRIDGE || (!BOOKMARKLET_MODE && localStorage.getItem(NEW_TAB_MODE_KEY) === 'false')) return null;
    try {
      if (BOOKMARKLET_NEWTAB) return window.open(browserOnlyGptUrl(GPT_URL), '_blank');
  ...
  async function handoffJob(job, say, userscriptMessage, preparedTab = null) {
    if (BOOKMARKLET_MODE) {
      const bookmarkletJob = { ...job, bookmarklet: true, newTab: BOOKMARKLET_NEWTAB };
      const payload = encodeTransfer(bookmarkletJob);
      const target = `${browserOnlyGptUrl(GPT_URL)}#${BOOKMARKLET_JOB_HASH}=${payload}`;
      if (BOOKMARKLET_NEWTAB) {
        let transferTab = preparedTab;
        try {
          if (!transferTab || transferTab.closed) transferTab = window.open(target, '_blank');
          else transferTab.location.href = target;
        } catch (error) { transferTab = null; }
        if (transferTab && !transferTab.closed) {
          say(`${userscriptMessage} GPT 새 탭에서 같은 북마클릿을 한 번 더 눌러주세요.`);
          try { transferTab.focus(); } catch (error) {}
          return;
        }
        say('새 탭이 차단되어 기존 같은 탭 방식으로 전환해요.', true);
      } else {
  ...
  async function init() {
    ...
    if (/zeta-ai\.io$/i.test(location.hostname)) {
      ...
      if (BOOKMARKLET_MODE && BOOKMARKLET_NEWTAB) {
        window.addEventListener('message', event => {
          try {
            const originHost = new URL(event.origin).hostname;
            if (!/(^|\.)chatgpt\.com$/i.test(originHost)) return;
          } catch (error) { return; }
          const packet = event.data;
          if (!packet || packet.source !== 'AUTO_KILLER_BOOKMARKLET' || packet.type !== 'result') return;
          const pending = packet.response;
          if (!pending || pending.room !== location.href.split('#')[0]) return;
          Promise.resolve(sharedStorage.set(RESPONSE_KEY, pending)).catch(() => {});
        });
