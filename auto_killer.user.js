// ==UserScript==
// @name         AUTO_KILLER OneClick (iPhone + Android)
// @namespace    local.zeta.gpt.oneclick.unified
// @version      2.24.4U
// @description  iPhone Safari Userscripts + Android Firefox에서 ZETA → 역병킬러 GPT → ZETA를 한 번의 작업 클릭으로 자동 왕복합니다.
...
(function () {
  ...
  const RESULT_HASH_KEY = 'akresult';
  const LOCAL_RESPONSE_KEY = 'zk_bookmarklet_zk_response_v2';
  ...
  async function insertPrompt(prompt, text) {
    ...
    // textarea 계열이면 native value setter를 우선 사용한다.
    ...
    // iOS Safari에서 execCommand가 무시될 때 contenteditable DOM 직접 입력 fallback.
    try {
      ...
      console.warn('[AUTO_KILLER OneClick] GPT 입력 fallback 실패', error);
      return false;
    }
  }
  ...
  function embeddedCore() {
    ...
    async function handoffJob(job, say, userscriptMessage, preparedTab = null) {
      ...
      if (ONECLICK_BRIDGE) {
        // Android는 실제 성공 확인된 2.23.13F 새 탭 경로를 그대로 유지한다.
        // iPhone/iPad Safari는 팝업 의존을 없애기 위해 같은 탭으로 이동한다.
        const outgoingJob = { ...job, newTab: !ONECLICK_IOS, oneclick: true };
      ...
      const outgoingJob = transferTab && !transferTab.closed ? { ...job, newTab: true } : job;
      await sharedStorage.set(JOB_KEY, outgoingJob);
      const conversationUrl = await sharedStorage.get(CONVERSATION_KEY, GPT_URL);
    ...
    function createGenerationPromptSettings(makeButton, say) {
      ...
      const addPreset = makeButton('사용자 프롬프트 저장', '#eceff1'); addPreset.style.cssText += 'align-self:flex-start;padding:5px 8px';
      addPreset.onclick = () => {
        ...
        presets.push({ id, title, content }); selectedPresets.add(id); localStorage.setItem(GENERATION_PRESETS_KEY, JSON.stringify(presets)); persist();
        newTitle.value = ''; newContent.value = ''; directCheck.checked = false; render(); say(`생성 프롬프트 「${title}」을 저장했어요.`);
      };
      const restore = makeButton('기본 생성 프롬프트 복구', '#fff', '#6b7280'); restore.style.cssText += 'align-self:flex-start;padding:5px 8px';
      restore.onclick = () => { builtins = { ...DEFAULT_GENERATION_PROMPTS }; deleted = new Set(); selectedBuiltins.clear(); localStorage.setItem(GENERATION_PROMPTS_KEY, JSON.stringify(builtins)); localStorage.setItem(GENERATION_DELETED_KEY, '[]'); persist(); render(); say('기본 생성 프롬프트 3개를 복구했어요.'); };
      render();
      root.append(label('기본 생성 프롬프트'), builtinList, label('사용자 생성 프롬프트'), presetList, directLabel, newTitle, newContent, addPreset, restore);
    ...
    function panel(mode) {
      ...
      if (mode === 'zeta') {
        ...
        const newTabToggle = makeButton(`새 탭 ${localStorage.getItem(NEW_TAB_MODE_KEY) === 'false' ? 'OFF' : 'ON'}`, '#fff');
        newTabToggle.style.display = BOOKMARKLET_MODE ? 'none' : 'inline-block';
        ...
        normalOnlyControls.push(openSettings, auto); if (!BOOKMARKLET_MODE) normalOnlyControls.push(newTabToggle); compactOnlyControls.push(compactExpand);
        ...
        resetGuide.textContent = '검토·생성·요약 설정을 선택해 초기화할 수 있어요. 사용자 프롬프트는 유지하거나 함께 삭제할 수 있어요.';
        resetGuide.style.cssText = 'color:#8a9099;font:500 10px/1.35 system-ui,sans-serif;word-break:keep-all';
        const resetButton = makeButton('초기화', '#fff', '#5f6670');
        ...
        resetSubText.textContent = '초기화할 설정을 모두 선택한 뒤 사용자 프롬프트 처리 방법을 골라주세요.';
        ...
        newTabToggle.onclick = () => {
          ...
          newTabToggle.textContent = `새 탭 ${enabled ? 'ON' : 'OFF'}`;
          say(enabled
        ...
        settings.append(
          ...
          categoryLabel('설정 초기화'), resetArea
        );
        row.append(review, generate, summarize, openSettings, auto, newTabToggle, compactExpand);
  ...
  function bootZeta() {
    // Firefox에서는 unsafeWindow로 page realm flag를 직접 전달한다.
    // iPhone Safari Userscripts에서 unsafeWindow가 없어도 core URL query로 동일 정보를 전달한다.
    try {
    ...
    const mirrorResponse = response => {
      if (!response?.id || response.room !== location.href.split('#')[0]) return false;
      lastResponseId = response.id;
      try {
        pageWindow.localStorage.setItem(LOCAL_RESPONSE_KEY, JSON.stringify(response));
        return true;
      } catch (error) {
        try {
          localStorage.setItem(LOCAL_RESPONSE_KEY, JSON.stringify(response));
          return true;
        } catch (innerError) {
          console.error('[AUTO_KILLER OneClick] ZETA localStorage 미러링 실패', innerError);
          return false;
        }
      }
    };

    // iPhone 같은 탭 왕복의 주 경로. GPT 결과를 URL hash에 싣고 돌아오므로
    // iOS Userscripts의 사이트 간 GM 공유 여부와 무관하게 결과를 회수할 수 있다.
    const resultMatch = location.hash.match(new RegExp(`(?:^#|[&#])${RESULT_HASH_KEY}=([^&]+)`));
    if (resultMatch) {
      const response = decodeTransfer(decodeURIComponent(resultMatch[1]));
      if (response?.id && response.room === location.href.split('#')[0]) {
        mirrorResponse(response);
        try { history.replaceState(null, '', response.room); } catch (error) {}
        // GM 저장이 함께 성공했던 경우의 중복 재수신 방지용 정리. 실패해도 hash 경로에는 영향 없음.
        gmDelete(RESPONSE_KEY).catch(() => {});
      }
    }
    ...
    const receiveResponse = async () => {
      try {
        ...
        if (!mirrorResponse(response)) return;
    ...
    // Android의 실제 성공 경로를 그대로 유지한다. iOS에서는 hash가 주 경로이고 이 폴링은 백업이다.
    ...
    // Android Firefox는 검증된 page realm 실행 방식을 사용한다.
    // Safari Userscripts는 unsafeWindow가 없는 경우 현재 실행 realm에서 번들 core를 직접 실행한다.
    try {
      if (typeof unsafeWindow === 'object' && unsafeWindow && typeof pageWindow.Function === 'function') {
        const source = `(${embeddedCore.toString()})();\n//# sourceURL=AUTO_KILLER_embedded_core_${VERSION}.js`;
        pageWindow.Function(source).call(pageWindow);
      } else {
        embeddedCore();
      }
    } catch (error) {
      console.error('[AUTO_KILLER OneClick] 번들 core 실행 실패', error);
      alert('AUTO_KILLER 본체 실행 실패: ' + (error?.message || error));
    }
  }
  ...
  async function bootGpt() {
    ...
    const watchResponse = submitted => {
      ...
      const timer = setInterval(async () => {
        ...
        // Android 새 탭은 2.23.13F에서 실제 성공한 GM → 원래 ZETA 탭 경로를 유지한다.
        if (submitted.newTab) {
          try {
            say('완료 · ZETA에 결과 전달 중…');
            await gmSet(RESPONSE_KEY, response);
            await sleep(500);
            say('완료 · 원래 ZETA 탭으로 돌아가는 중…');
            try { if (window.opener && !window.opener.closed) window.opener.focus(); } catch (error) {}
            window.close();
            // 브라우저가 스크립트 창 닫기를 거부한 경우에만 같은 탭 hash 복귀가 작동한다.
            setTimeout(() => {
              if (!document.hidden) location.replace(`${response.room.split('#')[0]}#${RESULT_HASH_KEY}=${encodeTransfer(response)}`);
            }, 900);
          } catch (error) {
            pageWindow.__AUTO_KILLER_ONECLICK_GPT_RUNNING__ = false;
        ...
        // iPhone/iPad 같은 탭은 hash 결과가 주 경로다. GM 저장은 성공하면 백업으로만 사용한다.
        say('완료 · ZETA로 돌아가는 중…');
        try { await gmSet(RESPONSE_KEY, response); } catch (error) {
          console.warn('[AUTO_KILLER OneClick] iOS GM 결과 백업 저장 실패, hash 복귀 계속', error);
        }
        await sleep(250);
        location.replace(`${response.room.split('#')[0]}#${RESULT_HASH_KEY}=${encodeTransfer(response)}`);
      }, 800);
    };

    // ChatGPT 내부 리로드/SPA 재초기화가 발생해도 같은 작업을 두 번 전송하지 않고 감시만 이어간다.
    if (job.stage === 'submitted') {
      say('페이지 전환 뒤 답변 감시를 이어가는 중…');
      watchResponse(job);
      return;
    ...
    if (!prompt) { pageWindow.__AUTO_KILLER_ONECLICK_GPT_RUNNING__ = false; fail('GPT 입력창을 못 찾았어요.'); return; }
    ...
    if (!inserted) {
      pageWindow.__AUTO_KILLER_ONECLICK_GPT_RUNNING__ = false;
      fail('GPT 입력창에 내용을 넣지 못했어요.');
      return;
    }
    await sleep(500);

    const submit = await waitFor('#composer-submit-button,button[data-testid="send-button"]', 10000);
    if (!submit) { pageWindow.__AUTO_KILLER_ONECLICK_GPT_RUNNING__ = false; fail('전송 버튼을 못 찾았어요.'); return; }
    let attempts = 0;
    while (submit.disabled && attempts++ < 30) await sleep(200);
    if (submit.disabled) { pageWindow.__AUTO_KILLER_ONECLICK_GPT_RUNNING__ = false; fail('전송 버튼이 활성화되지 않았어요.'); return; }

    say('자동 전송 · 답변 기다리는 중…');
    submit.click();
    watchResponse(submitted);
  }
